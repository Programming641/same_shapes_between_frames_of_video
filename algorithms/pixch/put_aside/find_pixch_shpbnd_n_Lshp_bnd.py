...
import os, sys
from libraries.cv_globals import proj_dir, debug, top_shapes_dir, top_images_dir, internal
...
pixch_bnd_n_Lshp_bnd_dir = top_shapes_dir + directory + "pixch_bnd_n_Lshp_bnd/"
...
for shapeid in pixch_shapes:
   ...
   pixch_shapes_boundaries[shapeid] = pixel_shapes_functions.get_boundary_pixels(cur_shape_pixels )
...
for pcount_shapes in mainim_stats:
   ...
   for shapeid in pcount_shapes[1]:
      ...
      # {'69738': {'x': 158, 'y': 196}, '69737': {'x': 157, 'y': 196},... }  for normal shapes
      # {'69738': [ pixel indexes ], ... } for intnl_spixcShp
      ...
      # [ (334, 199), (335, 199), ...]
      cur_shp_bnd_pixels = shapes_boundaries[ shapeid ]
      ...
      for pixch_shapeid in pixch_shapes_boundaries:
         ...
         same_area = any(item in s_locations[shapeid] for item in pixch_shapes_in_im_areas[pixch_shapeid])
         ...
         for pixch_pixel in pixch_shapes_boundaries[pixch_shapeid]:
...
# now that I have pixel change shape's boundary pixel matches with large normal shape, check to see if image 2 large shape's boundaries
...
for im_shapeid, pixch_shape in pixch_matches.items():
   ...
   for im2shapeid in im2shapes_boundaries:
      # getting all current shape's boundary pixels
      # [ (334, 199), (335, 199), ...]
      cur_shp_pixels = im2shapes_boundaries[ im2shapeid ]
      if len( cur_shp_pixels ) < 50:
         continue
      
      for pixch_shapeid, boundary_pixels in pixch_shape.items():
            
         cur_pixch_shapeid_found = False
   
         for bnd_pixel in boundary_pixels:   
            if bnd_pixel in cur_shp_pixels:
               if not cur_pixch_shapeid_found:
                  ...
                  im1and2_matches[ im_shapeid ][ cur_match_counter ].append( [ bnd_pixel ] )
                  
                  cur_pixch_shapeid_found = True
               else:
                  im1and2_matches[ im_shapeid ][ cur_match_counter ][2].append( bnd_pixel )
