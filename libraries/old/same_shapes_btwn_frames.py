...
def process_boundaries_vertically(original_boundary_pixels, compare_boundary_pixels, shape_ids, filenames):
   ...
   def get_compare_column(compare_boundary_pixels, compare_column_counter):
      ...
      pixel_ids_with_current_x = [ xy for xy in compare_boundary_pixels  if xy[0] == compare_column_counter]
   ...
   original_smallest_x = min( [ xy[0] for xy in original_boundary_pixels ] )
   original_largest_x = max( [ xy[0] for xy in original_boundary_pixels ] )
   
   original_shape_width = original_largest_x - original_smallest_x

   compare_smallest_x = min( [ xy[0] for xy in compare_boundary_pixels ] )
   compare_largest_x = max( [ xy[0] for xy in compare_boundary_pixels ] )
   ...
   for x in range(original_smallest_x, original_largest_x + 1):
      ...
      # pixel_ids_with_current_x contains all xy coordinate pairs that have the current running y value.
      pixel_ids_with_current_x = [ xy for xy in original_boundary_pixels  if xy[0] == x]
...
def process_boundaries(original_boundary_pixels, compare_boundary_pixels, shape_ids, filenames):
   ...
   def get_compare_row(compare_boundary_pixels, compare_row_counter):
      ...
      pixel_ids_with_current_y = [xy for xy in compare_boundary_pixels  if xy[1] == compare_row_counter]
   ...
   original_smallest_y = min( [ xy[1] for xy in original_boundary_pixels ] )
   original_largest_y = max( [ xy[1] for xy in original_boundary_pixels ] )
   
   original_shape_height = original_largest_y - original_smallest_y

   compare_smallest_y = min( [ xy[1] for xy in compare_boundary_pixels ] ) 
   compare_largest_y = max( [ xy[1] for xy in compare_boundary_pixels ] )
   ...
   for y in range(original_smallest_y, original_largest_y + 1):
      ...
      xy_with_current_y = [ xy for xy in original_boundary_pixels  if ( xy[1] ) == y ]    
