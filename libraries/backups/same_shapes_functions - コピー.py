...
def check_match( possible_match_pixels, pixels_to_match, im_size, possible_pixels_len ):
    
    def get_pixels_from_consecutives( consecutives, direction ):
        pixels = set()
        for current_on_xy in consecutives:
        
            for xy_consecutive in consecutives[current_on_xy]:
            
                if direction == "bottom":
                    cur_x = current_on_xy
                    cur_y = max(xy_consecutive)
                
                elif direction == "top":
                    cur_x = current_on_xy
                    cur_y = min(xy_consecutive)
                
                elif direction == "left":
                    cur_x = min(xy_consecutive)
                    cur_y = current_on_xy
                
                elif direction == "right":
                    cur_x = max(xy_consecutive)
                    cur_y = current_on_xy
                
                pixels.add( (cur_x, cur_y) )
    
        return pixels
    
    
    def get_further_pixels( pixels_at_direction, further_pos_xy, further_position ):
        pixels = set()
        
        for pixel_at_direction in pixels_at_direction:
            for cur_further_pos in range(1, further_position + 1):

                cur_further_pos_xy = ( pixel_at_direction[0] + ( further_pos_xy[0] * cur_further_pos ), pixel_at_direction[1] + ( cur_further_pos * further_pos_xy[1] ) )
                if cur_further_pos_xy[0] < 0 or cur_further_pos_xy[0] >= im_size[0] or cur_further_pos_xy[1] < 0 or cur_further_pos_xy[1] >= im_size[1]:
                    # out of image range
                    continue
                
                cur_further_pos_pindex = pixel_functions.convert_xy_to_pindex( cur_further_pos_xy, im_size[0] )
                
                pixels.add(cur_further_pos_pindex)
    
        return pixels
    
    
    def get_current_score( further_pixels, pixels_to_match, further_position ):
    
        if len(further_pixels) / len(pixels_to_match) <= 0.05:
            # not enough further_pixels to consider
            return 0

        overlapped_pixels = further_pixels.intersection( pixels_to_match )
        cur_score = len(overlapped_pixels) / len(further_pixels)
        if cur_score >= 0.5:
            return further_position * cur_score
        else:
            return 0


    
    # { on every x value: [ { y consecutive }, ... ]
    y_consecutives = pixel_shapes_functions.get_consec_pixels_on_x_or_y( possible_match_pixels, "y", im_size )
    x_consecutives = pixel_shapes_functions.get_consec_pixels_on_x_or_y( possible_match_pixels, "x", im_size )
    
    bottom_pixels = get_pixels_from_consecutives( y_consecutives, "bottom" )
    top_pixels = get_pixels_from_consecutives( y_consecutives, "top" )
    left_pixels = get_pixels_from_consecutives( x_consecutives, "left" )
    right_pixels = get_pixels_from_consecutives( x_consecutives, "right" )

    save_temp_im_fpath = top_shapes_dir + "videos/street3/resized/min1/" + "temp/test.png"
    shutil.copy(top_images_dir + "videos/street3/resized/min1/" + "31" + ".png", save_temp_im_fpath)
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", possible_match_pixels, pixels_rgb=(255,0,0) )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", pixels_to_match, pixels_rgb=(255,255,0) )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", possible_match_pixels, pixels_rgb=(255,0,0), save_filepath=save_temp_im_fpath )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", bottom_pixels, pixels_rgb=(255,255,0), input_im=save_temp_im_fpath )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", top_pixels, pixels_rgb=(255,255,0), input_im=save_temp_im_fpath )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", left_pixels, pixels_rgb=(255,255,0), input_im=save_temp_im_fpath )
    #image_functions.cr_im_from_pixels( "31", "videos/street3/resized/min1/", right_pixels, pixels_rgb=(255,255,0), input_im=save_temp_im_fpath )

    unmatch_score = 0
    for further_position in range(1,4):
        
        further_bottom_pixels = get_further_pixels( bottom_pixels, ( 0, 1 ), further_position )
        further_top_pixels = get_further_pixels( top_pixels, ( 0, -1 ), further_position)
        further_left_pixels = get_further_pixels( left_pixels, ( -1, 0 ), further_position )
        further_right_pixels = get_further_pixels( right_pixels, ( 1, 0 ), further_position )

        #image_functions.cr_im_from_pixels( "30", "videos/street3/resized/min1/", further_bottom_pixels, pixels_rgb=(100,0,255), input_im=save_temp_im_fpath )
        #image_functions.cr_im_from_pixels( "30", "videos/street3/resized/min1/", further_top_pixels, pixels_rgb=(100,0,255), input_im=save_temp_im_fpath )
        #image_functions.cr_im_from_pixels( "30", "videos/street3/resized/min1/", further_left_pixels, pixels_rgb=(100,0,255), input_im=save_temp_im_fpath )
        #image_functions.cr_im_from_pixels( "30", "videos/street3/resized/min1/", further_right_pixels, pixels_rgb=(100,0,255), input_im=save_temp_im_fpath )

        unmatch_score += get_current_score( further_bottom_pixels, pixels_to_match, further_position )
        unmatch_score += get_current_score( further_top_pixels, pixels_to_match, further_position )
        unmatch_score += get_current_score( further_left_pixels, pixels_to_match, further_position )
        unmatch_score += get_current_score( further_right_pixels, pixels_to_match, further_position )
...
def match_shape_while_moving_it2( im1pixels, im2pixels, im_size, min_colors, match_threshold=0.5, best_match=False, ignore_colors=False, return_all_matched=False ):
      ...
      for left_right in range( movement[0], movement[1], movement[2] ):
         ...
         for movement in movements:
            for down_up in range( movement[0], movement[1], movement[2] ):
               ...
               if len( matched_pixels ) / len( im1pixels ) >= match_threshold or len( matched_pixels ) / len( im2pixels ) >= match_threshold:
                  if best_match is True:
                     if len( matched_pixels ) > len( best_match_pixels ):
                     
                        match = check_match( matched_pixels, set( im2pixels.keys() ), im_size, len(moved_im1pixels) )
                        print("result of check_match is " + str(match) )
                        if match is False:
                           continue
...
def match_t_shape_w_nbrs_while_moving_it( im1pixels, im2pixels, target_im1pixels, target_im2pixels, im_size, min_colors, added_shapes=None, match_threshold=0.5, 
   ...
   def get_match_difficulty():
      ...
      return match_difficulty_variable
   ...
   if added_shapes is not None:
      match_difficulty_variable = get_match_difficulty()
      ...
      for left_right in range( movement[0], movement[1], movement[2] ):
         if left_right >= start_move and left_right > skip_xy["pos_x"]:
            ...
            if left_right == end_move:
               reset_skip( skip_xy, "pos_x" )
            break
         elif left_right <= negative_start_move and left_right < skip_xy["neg_x"]:
            if left_right == -end_move:
               reset_skip( skip_xy, "neg_x" )
            break
            ...
            for down_up in range( movement[0], movement[1], movement[2] ):
               if down_up >= start_move and down_up > skip_xy["pos_y"]:
                  ...
                  if down_up == end_move:
                     reset_skip( skip_xy, "pos_y" )
                  break
               elif down_up <= negative_start_move and down_up < skip_xy["neg_y"]:
                  if down_up == -end_move:
                     reset_skip( skip_xy, "neg_y" )
                  break
