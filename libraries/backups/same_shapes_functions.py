...
# im1pixels and im2pixels are NOT the shape coordinate pixels.
#
def match_t_shape_w_nbrs_while_moving_it( im1pixels, im2pixels, target_im1pixels, target_im2pixels, im_size, min_colors, match_threshold=0.5, ignore_colors=False ):
   ...
   target_match_threshold = match_threshold + 0.1
         ...
         for movement in movements:
            for down_up in range( movement[0], movement[1], movement[2] ):
               ...
               if len( matched_pixels ) / len( im1pixels ) >= match_threshold:
                  ...
                  matched_pixels = get_matched_pixels( moved_target1pixels, target_im2pixels )
                  
                  #image_functions.cr_im_from_pixels( "26", "videos/street3/resized/min1/", matched_pixels, pixels_rgb=(255,0,255) ) 
                  if len( matched_pixels ) / len( moved_target1pixels ) >= target_match_threshold:
                     return True
...
def check_by_pixels_attachment( common_nbr_matches, im_shapes, app_im_shapes_nbr_pixels, im_size, 
   ...
   for neighbor_match in common_nbr_matches:
      ...
      #image_functions.cr_im_from_pixels( "25", "videos/street3/resized/min1/", im1neighbor_pixels, pixels_rgb=(255,0,0) )
      #image_functions.cr_im_from_pixels( "26", "videos/street3/resized/min1/", im2neighbor_pixels, pixels_rgb=(0,0,255) )           
      ...
      image_functions.cr_im_from_pixels( "26", "videos/street3/resized/min1/", moved_im1boundaries, pixels_rgb=(255,0,0), save_filepath=im1save_filepath )  
      im2save_filepath = top_shapes_dir + "videos/street3/resized/min1/temp/test3.png"
      image_functions.cr_im_from_pixels( "26", "videos/street3/resized/min1/", app_im_shapes_boundaries[1], pixels_rgb=(0,0,255), save_filepath=im2save_filepath )  
...
def match_by_mov_shape_against_im( im_shapes_colors, im_shapeids_by_pindex, whole_pixels, target_pixels, im_size, min_colors, im_shapes, nont_neighbor_shapeids, 
                                   return_shapeids=True, return_matched_pixels=False ):

   move_amount = 16
   match_threshold = 0.55
   
   most_matched_score = 0
   #  [ [ {matched image shapeids}, {matched target shapeids } ],  [ {matched image shapeids}, {matched non target shapeids } ] ]
   most_matched_shapeids = []
   most_matched_nont_pixels = set()
   most_matched_t_pixels = set()
   most_matched_movement = None
   
   
   def get_im_data_matched_score( matched_imd_shapeids, t_nont_pixels_len, nont=False ):
      nont_match_threshold = 0.25
      matched_nont_shapes_counts = 0
      
      # get most matched image data shapes    
      # [ matched score, [ [ matched image data shapeids ], [ matched target or non target shapeids ] ]      
      most_matched_imd_sids = [ 0, [ set(), set() ] ]      
      for shapeid in matched_imd_shapeids:
         
         # if non target pixels contain multiple shapes, get most matched nont shape
         if nont is False:
         
            imd_matched_percent = matched_imd_shapeids[shapeid] / len( im_shapes[1][shapeid] )
            if imd_matched_percent < match_threshold:
               continue         
         
            t_nont_matched_percent = matched_imd_shapeids[shapeid] / t_nont_pixels_len
            if t_nont_matched_percent < match_threshold:
               continue
         
            cur_score = imd_matched_percent + t_nont_matched_percent
         
            if cur_score > most_matched_imd_sids[0]:
               most_matched_imd_sids[0] = cur_score
               most_matched_imd_sids[1][0] = {shapeid}
               most_matched_imd_sids[1][1] = nont_neighbor_shapeids
            elif cur_score == most_matched_imd_sids[0]:
               most_matched_imd_sids[1][0].add(shapeid)
               

            #if shapeid == 30848 or shapeid == 32031:
            #   print("shapeid " + str(shapeid) + " imd_matched_percent " + str(imd_matched_percent) + " t_nont_matched_percent " + str(t_nont_matched_percent) )

         #image_functions.cr_im_from_shapeslist2( "26", "videos/street3/resized/min1/", most_matched_imd_sids[1] )            

         else:

            matched_nont_shapeids = set( matched_imd_shapeids[shapeid] )
            cur_nont_matched_shapeids = set()
            nont_matched_percent = 0
            
            for nont_shapeid in matched_nont_shapeids:
               cur_count = matched_imd_shapeids[shapeid].count( nont_shapeid )

               imd_matched_percent = cur_count / len( im_shapes[1][shapeid] )
               if imd_matched_percent < match_threshold:
                  continue   

               cur_nont_matched_percent = cur_count / len( im_shapes[0][nont_shapeid] )
               
               if cur_nont_matched_percent > nont_matched_percent:
                  nont_matched_percent = cur_nont_matched_percent
                  cur_nont_matched_shapeids = { nont_shapeid }
               elif cur_nont_matched_percent == nont_matched_percent:
                  cur_nont_matched_shapeids.add( nont_shapeid )
 
            if nont_matched_percent < match_threshold:
               continue
            
            matched_nont_shapes_counts += 1
            
            cur_score = imd_matched_percent + nont_matched_percent

            most_matched_imd_sids[0] += cur_score
            most_matched_imd_sids[1][0].add(shapeid)     
            most_matched_imd_sids[1][1] |= cur_nont_matched_shapeids
      
      if nont is True:      
         if matched_nont_shapes_counts == 0 or matched_nont_shapes_counts / len(matched_imd_shapeids) < nont_match_threshold:
            most_matched_imd_sids[0] = 0
         else:
            most_matched_imd_sids[0] = most_matched_imd_sids[0] / matched_nont_shapes_counts
   
      return most_matched_imd_sids
      ...
      for move_RL in range( start_position_RL, move_amount ):
         ...
         for pos_neg_UD in [ 1, - 1 ]:
            ...
            for move_UD in range( start_position_UD, move_amount ):
               ...
               for whole_pixel in whole_pixels:
                  ...
                  if pixel_matched is True:
                     ...
                     if whole_pixel in target_pixels:
                        ...
                        non_t_matched_pixels.add(whole_pixel) 
               ...
               # [ matched score, [ {matched image shapeids}, {matched target or non target shapeids } ] ]
               t_imd_matched_score = get_im_data_matched_score( matched_t_im_shapeids, len(target_pixels) )
               non_t_imd_matched_score = get_im_data_matched_score( matched_non_t_im_shapeids, len(whole_pixels) - len(target_pixels), nont=True  )
               #print("t_imd_matched_score " + str(t_imd_matched_score) + " non_t_imd_matched_score " + str(non_t_imd_matched_score) )
               #print( "target_matched_mv_pixels " + str( len(target_matched_mv_pixels) ) + " target_pixels " + str(len(target_pixels)) + 
               #       " matched_pixels " + str(len(matched_pixels)) + " whole_pixels " + str(len(whole_pixels)) )

               print("t_imd_matched_score " + str(t_imd_matched_score) + " non_t_imd_matched_score " + str(non_t_imd_matched_score) )
               
               if t_imd_matched_score[0] == 0 or  non_t_imd_matched_score[0] == 0:
                  # not matched 
                  continue
               
               cur_score = t_imd_matched_score[0] + non_t_imd_matched_score[0]
               if return_shapeids is True:

                  if cur_score > most_matched_score:
                     most_matched_score = cur_score
                     
                     most_matched_shapeids = [ t_imd_matched_score[1], non_t_imd_matched_score[1] ]
  
                     #image_functions.cr_im_from_pixels( "26", "videos/street3/resized/min1/", target_matched_mv_pixels, pixels_rgb=(0,255,255) ) 

               elif return_matched_pixels is True:
                  if cur_score > most_matched_score:
                     most_matched_score = cur_score
                     most_matched_nont_pixels = non_t_matched_pixels
                     most_matched_t_pixels = target_matched_mv_pixels
                     most_matched_movement = ( move_RL, move_UD )
   ...
   if return_shapeids is True:
      ...
      return most_matched_nont_pixels, most_matched_t_pixels, most_matched_movement
